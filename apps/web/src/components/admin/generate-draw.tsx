...
export function GenerateDraw({ eventId }: GenerateDrawProps) {
  ...
  const hasInsufficientPlayers = confirmedCount > 0 && playersInDraw < confirmedCount && !hasExcessPlayers
    ...
    <div className="space-y-6">
      ...
      {hasInsufficientPlayers && (
        <Card className="border-yellow-500 bg-yellow-50 dark:bg-yellow-950/20">
          <CardHeader>
            <CardTitle className="text-yellow-800 dark:text-yellow-200">ℹ️ Insufficient Players</CardTitle>
            <CardDescription className="text-yellow-700 dark:text-yellow-300">
              You have {confirmedCount} confirmed players, but only {playersInDraw} can form complete teams (multiple of 4).
              <br />
              {waitlistedPlayers > 0 && (
                <>
                  <strong>{waitlistedPlayers} player{waitlistedPlayers !== 1 ? 's' : ''}</strong> will be waitlisted.
                  <br />
                </>
              )}
              You have capacity for {maxPlayers} players ({mastersCourts.length} MASTERS courts + {explorersCourts.length} EXPLORERS courts) but only need {courtsNeeded} courts.
              <br />
        ...
        <Card className="border-orange-500 bg-orange-50 dark:bg-orange-950/20">
          <CardHeader>
            ...
            <CardDescription className="text-orange-700 dark:text-orange-300">
              ...
              <br />
              <strong>{playersInDraw} players</strong> (top-rated) will be included in the draw, and <strong>{waitlistedPlayers} players</strong> will be waitlisted.
              <br />
      ...
      {!hasExcessPlayers && unusedCourts > 0 && playersInDraw > 0 && (
        <Card className="border-blue-500 bg-blue-50 dark:bg-blue-950/20">
          <CardHeader>
            <CardTitle className="text-blue-800 dark:text-blue-200">💡 Court Optimization</CardTitle>
            <CardDescription className="text-blue-700 dark:text-blue-300">
              With {playersInDraw} players, you only need {courtsNeeded} court{courtsNeeded !== 1 ? 's' : ''}.
              <br />
              You have {mastersCourts.length} courts for MASTERS and {explorersCourts.length} courts for EXPLORERS.
              <br />
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            {tierRules.masterCount !== undefined ? (
              ...
              <Badge variant="outline">{tierRules.masterPercentage}% MASTERS / {100 - tierRules.masterPercentage}% EXPLORERS</Badge>
            ) : (
          ...
          <div className="grid grid-cols-2 gap-4 p-3 bg-white dark:bg-gray-900 rounded-lg border">
            <div>
              <div className="text-sm font-medium text-yellow-700 dark:text-yellow-400">MASTERS</div>
              <div className="text-2xl font-bold">{Math.floor(playersInDraw * (tierRules.masterPercentage || 50) / 100)}</div>
              <div className="text-xs text-muted-foreground">players expected</div>
            </div>
            <div>
              <div className="text-sm font-medium text-green-700 dark:text-green-400">EXPLORERS</div>
              <div className="text-2xl font-bold">{playersInDraw - Math.floor(playersInDraw * (tierRules.masterPercentage || 50) / 100)}</div>
              <div className="text-xs text-muted-foreground">players expected</div>
            </div>
          </div>
            ...
            {tierRules.mastersTimeSlot && (
              <div className="p-3 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  {mastersCourts.length} court{mastersCourts.length !== 1 ? 's' : ''} (capacity: {mastersCapacity} players)
                </div>
              </div>
            )}
            {tierRules.explorersTimeSlot && (
              <div className="p-3 bg-green-100 dark:bg-green-900/30 rounded-lg">
                <div className="text-sm font-medium text-green-800 dark:text-green-200 mb-1">EXPLORERS Time Slot</div>
                <div className="text-sm">
                  {tierRules.explorersTimeSlot.startsAt} - {tierRules.explorersTimeSlot.endsAt}
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  {explorersCourts.length} court{explorersCourts.length !== 1 ? 's' : ''} (capacity: {explorersCapacity} players)
                </div>
              </div>
            )}
          </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">Create balanced teams based on player ratings</p>
              </div>
              <Switch
                id="balance-strength"
                checked={constraints.balanceStrength}
                onCheckedChange={(checked) => setConstraints({ ...constraints, balanceStrength: checked })}
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">Allow Masters and Explorers to play together if needed</p>
              </div>
              <Switch
                id="tier-mixing"
                checked={constraints.allowTierMixing}
                onCheckedChange={(checked) => setConstraints({ ...constraints, allowTierMixing: checked })}
              />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {event.confirmedPlayers?.slice(playersInDraw).map((player: any, index: number) => (
                <div key={player.id} className="flex items-center justify-between p-2 border rounded bg-orange-50 dark:bg-orange-950/20">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">#{playersInDraw + index + 1}</span>
                    <span className="text-sm">{player.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="text-xs">
        ...
        <Card className="border-destructive">
          <CardContent className="pt-6">
            <p className="text-destructive">
              Error: {generateDrawMutation.error instanceof Error ? generateDrawMutation.error.message : "Failed to generate draw"}
            </p>
          </CardContent>
        </Card>
      )}
